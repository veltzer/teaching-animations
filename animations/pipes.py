...
class PipesAnimation(VoiceoverScene):
    ...
    def draw_actors(self):
        writer_box = RoundedRectangle(width=2.4, height=1.2, corner_radius=0.15, color=BLUE_C, fill_opacity=0.5)
        writer_box.move_to(LEFT * 5)
        writer_lbl = Text("writer", font_size=20).move_to(writer_box.get_center() + UP * 0.2)
        writer_fd = Text("fd[1]", font_size=18, color=YELLOW, font="Monospace").move_to(writer_box.get_center() + DOWN * 0.2)
        writer = VGroup(writer_box, writer_lbl, writer_fd)

        reader_box = RoundedRectangle(width=2.4, height=1.2, corner_radius=0.15, color=GREEN_C, fill_opacity=0.5)
        reader_box.move_to(RIGHT * 5)
        reader_lbl = Text("reader", font_size=20).move_to(reader_box.get_center() + UP * 0.2)
        reader_fd = Text("fd[0]", font_size=18, color=YELLOW, font="Monospace").move_to(reader_box.get_center() + DOWN * 0.2)
        reader = VGroup(reader_box, reader_lbl, reader_fd)

        # Buffer in the middle: 8 slots
        slots = VGroup()
        for i in range(8):
            slot = Rectangle(width=0.55, height=0.7, color=GREY, fill_opacity=0.15)
            slot.move_to(LEFT * 1.95 + RIGHT * (i * 0.6))
            slots.add(slot)

        buf_lbl = Text("kernel pipe buffer (capacity = 8)", font_size=18, color=GREY).next_to(slots, UP, buff=0.4)
    ...
    def _fill_slot(self, slot, char, color):
        ...
        text = Text(char, font_size=22, font="Monospace", color=WHITE).move_to(slot.get_center())
        self.play(Transform(slot, new_slot), FadeIn(text, shift=DOWN), run_time=0.25)
        return text

    def _empty_slot(self, slot, text):
        new_slot = Rectangle(width=slot.width, height=slot.height, color=GREY, fill_opacity=0.15).move_to(slot.get_center())
    ...
    def write_some(self, writer, slots):
        with self.voiceover(text="The writer calls write of three bytes — H, I, exclamation."):
            self.play(Indicate(writer, color=BLUE_A, scale_factor=1.05))
            t1 = self._fill_slot(slots[0], "H", BLUE)
            t2 = self._fill_slot(slots[1], "i", BLUE)
            t3 = self._fill_slot(slots[2], "!", BLUE)
    ...
    def read_some(self, reader, slots):
        with self.voiceover(text="The reader calls read of three bytes — and gets H, I, exclamation, in order."):
            self.play(Indicate(reader, color=GREEN_A, scale_factor=1.05))
    ...
    def fill_to_block(self, writer, buf, slots):
        with self.voiceover(
            ...
            for slot, c in zip(slots, chars):
                self._fill_slot(slot, c, BLUE)

        block_lbl = Text("write() blocks → buffer full", font_size=22, color=RED).next_to(buf, DOWN, buff=0.4)
        with self.voiceover(
    ...
    def drain_to_block(self, reader, buf, slots):
        ...
        block_lbl = Text("read() blocks → buffer empty", font_size=22, color=RED).next_to(buf, DOWN, buff=0.4)
        with self.voiceover(
