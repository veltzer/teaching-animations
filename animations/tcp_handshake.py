...
class TcpHandshakeAnimation(VoiceoverScene):
    ...
    def draw_endpoints(self):
        client_box = RoundedRectangle(width=2.4, height=1.2, corner_radius=0.15, color=BLUE_C, fill_opacity=0.5)
        client_box.move_to(LEFT * 5 + UP * 2.5)
        client_lbl = Text("client", font_size=22).move_to(client_box.get_center() + UP * 0.2)
        client_state = Text("CLOSED", font_size=16, color=GREY, font="Monospace").move_to(client_box.get_center() + DOWN * 0.2)
        client = VGroup(client_box, client_lbl, client_state)

        server_box = RoundedRectangle(width=2.4, height=1.2, corner_radius=0.15, color=GREEN_C, fill_opacity=0.5)
        server_box.move_to(RIGHT * 5 + UP * 2.5)
        server_lbl = Text("server", font_size=22).move_to(server_box.get_center() + UP * 0.2)
        server_state = Text("LISTEN", font_size=16, color=GREEN, font="Monospace").move_to(server_box.get_center() + DOWN * 0.2)
        server = VGroup(server_box, server_lbl, server_state)

        # Vertical timelines
        client_line = DashedLine(client_box.get_bottom(), client_box.get_bottom() + DOWN * 5.5, color=BLUE, dash_length=0.15)
        server_line = DashedLine(server_box.get_bottom(), server_box.get_bottom() + DOWN * 5.5, color=GREEN, dash_length=0.15)
    ...
    def _packet(self, src, dst, y_offset, label_text, color):
        ...
        mid = (start + end) / 2
        label = Text(label_text, font_size=18, color=color, font="Monospace").move_to(mid + UP * 0.05)
        # Slight offset so it doesn't overlap the arrow
        label.shift(UP * 0.25)
        return arrow, label

    def _set_state(self, endpoint, new_text, color):
        new_state = Text(new_text, font_size=16, color=color, font="Monospace").move_to(endpoint.state.get_center())
        self.play(Transform(endpoint.state, new_state), run_time=0.4)

    def syn(self, client, server):
        arrow, label = self._packet(client, server, 0.5, "SYN  seq=100", BLUE)
        ...
        with self.voiceover(
            ...
            self._set_state(client, "SYN_SENT", YELLOW)
    ...
    def syn_ack(self, client, server):
        ...
        with self.voiceover(
            ...
            self.play(GrowArrow(arrow), Write(label))
            self._set_state(server, "SYN_RECVD", YELLOW)

    def ack(self, client, server):
        arrow, label = self._packet(client, server, 2.8, "ACK  ack=501", BLUE)
        ...
        with self.voiceover(
            ...
            self.play(GrowArrow(arrow), Write(label))
            self._set_state(client, "ESTABLISHED", GREEN)
    ...
    def connection_open(self, client, server):
        with self.voiceover(
            ...
            self._set_state(server, "ESTABLISHED", GREEN)

        banner = Text("connection open — data may flow", font_size=22, color=YELLOW).to_edge(DOWN, buff=0.6)
